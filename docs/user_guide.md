...
![](media/real-domains.png)

A real Src-family kinase family (SRC, YES, FYN, FGR, HCK, LYN, LCK, BLK) with
its tree and real InterProScan domain annotations generated by
`react-msaview-cli interproscan`. Zoomed out to the full length, the shared
SH3 (blue), SH2 (green), and tyrosine-kinase catalytic (red) domain
architecture lines up across every member of the family.

![](media/large-tree.png)
...
straight from the Stockholm file. This shows the canvas tiling holding up well
past toy-sized data.
