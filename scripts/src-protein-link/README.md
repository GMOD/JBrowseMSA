...
gene on hg38 so the two views share coordinates through a codon mapping.
Clicking a residue in the MSA navigates the genome to its codon, and hovering
the gene highlights the MSA column. `generate.mjs` builds the link from a public
source.

## How the link works

The link is a single declarative JBrowse session spec
(`?config=…&session=spec-…`) with two views:
...
- **jbrowse-components**: `LaunchView-LinearGenomeView` must forward a spec `id`
  to the created view so `connectedViewId` can reference it. Without that, the
  spec's LGV gets an auto-generated id and the connection can't resolve. The
  forwarding is on `main`, and the docs link targets
  `jbrowse.org/code/jb2/main`.
