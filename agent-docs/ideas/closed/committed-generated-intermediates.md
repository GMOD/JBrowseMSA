...
`scripts/tp53-protein-link/.gitignore` and `scripts/braf-protein-link/.gitignore`
hold `work/`, and the other two generators never write a work directory.
`git ls-files 'scripts/*/work/*' 'scripts/examples-gen/build/*'` returns
nothing, and neither directory exists in a clean checkout.

The `.aln` files still tracked under `packages/app/public/data/` are a separate
case. They are the demo app's served data, the deployed site loads them, and the
diff-gated `pnpm figures` / `pnpm screenshots` runs depend on them.
