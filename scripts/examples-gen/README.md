...
| `corona_fse.stock` | Coronavirus frameshift element (Rfam RF00507 seed subset) | a pseudoknot: `SS_cons` writes it `A`/`a` because it crosses stem 1 rather than nesting in it, and the arc track draws the crossing | Baranov et al. 2005, Virology             |     |
...
`accession/coords` ids, same as the lysine example. `corona_fse.stock` is 18 of
the 51 RF00507 seed rows, one per interesting lineage across the four
coronavirus genera, and is the one that relabels its rows:
`NC_004718.3/13399-13476` says nothing and `SARS-CoV` says everything. Each
label's source row stays in the file as a `#=GS <label> AC <id>` line, so the
subset still names exactly what it took.

## Arcs

The viewer draws pairs of positions as arcs (see `docs/layers.md`). An RNA
Stockholm needs no help — `SS_cons` already pairs its columns — but a protein
pairing arrives as a `columnTracks` entry, and the examples here take theirs
from the same accession the sequence came from. The insulin example's three
disulfide bonds are the `Disulfide bond` features of UniProt P01308, read off
the entry and written into the component as residue pairs of the `Human` row:
